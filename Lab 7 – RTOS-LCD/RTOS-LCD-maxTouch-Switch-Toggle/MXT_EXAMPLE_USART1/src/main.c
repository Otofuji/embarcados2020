...
int process_touch(t_but botoes[], touchData touch, uint32_t n){
	for (int i = 0; i < n; i++) {
		if (botoes[i].y - (botoes[i].height/2) <= touch.y && botoes[i].y + (botoes[i].height/2) >= touch.y) {
			if (botoes[i].x - (botoes[i].width/2) <= touch.x && botoes[i].x + (botoes[i].width/2) >= touch.x) {
				return i;
			}
		}
	}
